...
Once you call `SetThreadDesktop(){:cpp}`, everything you spawn goes to that desktop. The `lpDesktop` field in `STARTUPINFO` controls which desktop a process starts in.
...
Using `BitBlt(){:cpp}` on the desktop DC only gets what's currently visible on top, so minimized or obscured windows don't show up. Better to enumerate windows individually:
...
Start from the bottom window with `GW_HWNDLAST`, walk upward with `GW_HWNDPREV`. `PrintWindow(){:cpp}` captures each window's content even if it's minimized or behind other windows.
...
Unchanged pixels get marked as RGB(255, 174, 201) so the server skips them. If every pixel matches, send 0 and skip transmission entirely. Compress with `RtlCompressBuffer(){:cpp}` using LZNT1 before sending.
...
`WindowFromPoint(){:cpp}` gives you the top-level window. Convert to client coordinates with `ScreenToClient(){:cpp}`, then walk the child hierarchy with `ChildWindowFromPoint(){:cpp}` until you hit the actual control.

For window dragging, check `WM_NCHITTEST` to see if the mouse is on the title bar or window edge. Then call `MoveWindow(){:cpp}` directly with adjusted coordinates:
